...
function Editproduct() {
    ...
    useEffect(()=>{
        ...
        changeprice(location.state.price);
        changedesc(location.state.desc);
        changequantity(location.state.quantity);
        loadWeb3();
        loadblockchaindata();
    },[])
    return (
            ...
            <div className="w-3/4 mx-auto my-5">
                ...
                <div className="flex flex-col w-full justify-center items-center">
                    ...
                    <button type="button" className="bg-white text-black w-24 rounded-lg border-0 px-3 py-2 my-2 mx-8" style={{'border':'1px solid black'}} disabled={submit_status} onClick={async (e) => {
                        console.log(account);
                        try {
                            await ecommerce.methods.editproduct(location.state.id,title,price,desc,quantity,imagearray[0]).send({from:account}).on('receipt', function(receipt){
                                alert("Product has been edited in the blockchain")
                            });
                        } catch (error) {
                            console.log(error);
                        }
